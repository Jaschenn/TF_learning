...
def img2gray(i_img):
    ...
    i_img = cv2.adaptiveThreshold(i_img, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 17, 8)  # 二值化
...
def save_img(img,lable):
    cv2.imwrite(os.path.join(output_dir, lable + "_" + str(random.randint(0, 9999999)) + ".png"), img)
