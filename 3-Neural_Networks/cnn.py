...
num_class = 10
# 为图像设置占位符
x= tf.placeholder("float",[None,img_size,img_size,1],"x")# None代表的是可以接受任意数量的图像
# 为标签设置占位符
y = tf.placeholder("float",[None,num_class],"y")# 形状为[None,num_class],类行为float
...
def model(X,w,w2,w3,w4,w_o,p_keep_conv,p_keep_hidden):
    conv1 = tf.nn.conv2d(X,w,[1,1,1,1],padding="SAME")# tf.nn.conv2d进行卷积操作,对于每一个维度都设置称为1。padding是设置图像边界被0填充，保证输出的大小一致
